...
export default defineComponent({
  ...
  data(){
    return {
      ...
      tubeIdentifiers:[
        ['404-5', '402-16', '401-9', '404-12', '401-16', '403-5', '401-11', '404-13', '403-12'],
        ['402-4', '402-15', '403-6', '404-4', '401-8', '404-6', '404-14', '401-12', '402-14'],
        ['402-5', '403-10', '403-7', '401-7', '403-1', '404-15'],
        ['401-13', '402-6', '402-13', '403-9', '404-16', '401-6', '404-7', '403-16', '403-4'],
        ['401-14', '402-12', '402-1', '403-15', '402-7', '403-8'],
        ['401-5', '403-3', '401-15', '402-11', '402-8', '403-14', '401-1', '401-2', '404-9'],
        ['402-2', '404-10', '403-13', '402-9', '404-11', '401-3','401-10', '402-10', '403-2']
      ].flat(),
    }
  },
  components: {
  ...
  methods:{
    async fetchData() {
      try {
        // http://218.92.43.42:14300/lsgn/api/get/fyzn/meteInfo?startTime=2026-03-25T12:25&endTime=2026-03-25T12:26
        ...
        // const newThermData = thermResponse.data;
    ...
    async fetchProjectTitle() {
      try {
        const response = await axios.get('http://127.0.0.1/lsgn/api/project/config/getInfo?configKey=project_title');
    ...
    updateHeatLossValues() {
      console.log("更新单元热损失")
      this.measuredUnitHeatLoss = parseFloat((129.41 + Math.random()).toFixed(2));
      this.unitHeatLoss = parseFloat((131.29 + Math.random()).toFixed(2));
    },
...
<template>
    ...
    <div class="iot-map-container">
      ...
      <el-breadcrumb :separator-icon="ArrowRight" style="font-size: 1rem;margin-top: 2.5rem; margin-left: 3.5rem;">
        ...
        <el-breadcrumb-item>点位一</el-breadcrumb-item>
      </el-breadcrumb>

      <SevenLayerTemperatureTable
          :thermData="thermData"
          :mediumTemperature="thermPointData.data[1].temperature"
      ...
      <div class="legend-container">
        ...
        <a style="color: #ffa200;font-size: 0.8rem;" >直管、弯头处热电偶分布示意图</a>
        <div class="legend-item">
          <img src="@/assets/images/iot/管网截面图.jpeg" style="width: 12rem;height: 10rem;">
        </div>
        <a style="color: #ffa200;font-size: 0.8rem;">补口处热电偶分布示意图</a>
        <div class="legend-item">
          <img src="@/assets/images/iot/补口位置热电偶分布.jpeg" style="width: 13rem;height: 11rem; ">
        </div>
        <div class="description">
          <p>
            表格中的展示的数据是:<p style="color: #ffa200;">点位一     跨南中心河</p>各层温度数据和经过公式计算得出的保温效果数据总览，此外根据表格中计算出的理论值设定报警阈值，保温层温度超过理论值则报警。
          </p>
        </div>
      </div>
